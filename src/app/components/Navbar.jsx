...
//     Services: () => <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><path d="M14.7 6.3a1 1 0 0 0 0 1.4l1.6 1.6a1 0 0 0 1.4 0l3.77-3.77a6 6 0 0 1-7.94 7.94l-6.91 6.91a2.12 2.12 0 0 1-3-3l6.91-6.91a6 6 0 0 1 7.94-7.94l-3.76 3.76z" /></svg>,
...
// const LandingPage = () => {
//     const router = useRouter(); // ✅ Router import kiya
...
//     // ✅ Hash handling - sirf same-page sections ke liye
//     useEffect(() => {
//         const handleHashChange = () => {
//             const hash = window.location.hash.replace('#', '');
//             // Sirf valid section IDs ko hi handle karein
//             const validSections = ['home', 'about', 'work'];
...
//     // ✅ Intersection Observer - sirf current page ke sections track kare
//     useEffect(() => {
...
//                     const validSections = ['home', 'about', 'work'];
//                     // Sirf valid sections ke liye hi update karein
//                     if (validSections.includes(id)) {
//                         setActiveSection(id);
//                         // URL hash update - sirf agar same page par hain
...
//     // ✅ FIXED: Navigation handler - Page links vs Hash links alag handle
//     const handleNavigation = (e, href, id) => {
//         e.preventDefault();
        
//         // ✅ Page links (/contact, /services) ke liye Next.js router use karein
//         if (href.startsWith('/')) {
//             router.push(href);
//             setIsMenuOpen(false);
//             return;
...
//         // ✅ Hash links (#home, #about) ke liye smooth scroll
...
//     // ✅ Nav Links - Page links ke liye id: null, hash links ke liye id: section-name
//     const navLinks = [
//         { name: 'Home', href: '#home', id: 'home', icon: <Icons.Home />, isPageLink: false },
//         { name: 'About', href: '#about', id: 'about', icon: <Icons.About />, isPageLink: false },
//         { name: 'Work', href: '#work', id: 'work', icon: <Icons.Work />, isPageLink: false },
...
//                                             !link.isPageLink && activeSection === link.id 
//                                                 ? 'text-[#e1c693]'
...
//                                             !link.isPageLink && activeSection === link.id ? 'bg-[#e1c693]/10 text-[#e1c693]' : ''
//                                         }`}
//                                     >
//                                         <a 
//                                             href={link.href} 
...
//   useEffect(() => {
//     const handleEscape = (e) => { if (e.key === "Escape") setShowThemePopup(false); };
//     if (showThemePopup) {
//       document.addEventListener("keydown", handleEscape);
//       document.body.style.overflow = "hidden";
//     }
...
//     if (el) {
//       const offsetTop = targetId === "home" ? 0 : el.offsetTop - 120;
//       window.scrollTo({ top: offsetTop, behavior: "smooth" });
//     }
//     setIsMenuOpen(false);
//   };
...
//               {navLinks.map((link) => (
//                 <li key={link.name}>
//                   <a href={link.href} onClick={(e) => handleNavigation(e, link.href, link.id)} className={`flex items-center gap-2 cursor-pointer transition-colors duration-300 ${isActiveLink(link) ? "text-[#e1c693]" : (isDark ? "text-gray-400 hover:text-[#e1c693]" : "text-gray-600 hover:text-[#a78b54]")}`}>
//                     {link.icon} {link.name}
//                   </a>
//                 </li>
//               ))}
...
//             <div className="flex items-center flex-shrink-0 cursor-pointer z-10 gap-3">
//               <button onClick={() => setShowThemePopup(true)} className={`p-2 rounded-md transition-colors duration-300 ${isDark ? "hover:bg-white/10 text-[#e1c693]" : "hover:bg-gray-100 text-[#a78b54]"}`} aria-label="Toggle Theme">
//                 {isDark ? <Sun className="w-5 h-5" /> : <Moon className="w-5 h-5" />}
//               </button>
//               <button onClick={() => setIsMenuOpen(!isMenuOpen)} className={`md:hidden p-2 rounded-md transition-colors ${isDark ? "text-gray-300 hover:bg-white/5" : "text-gray-800 hover:bg-gray-100"}`}>
//                 {isMenuOpen ? <Icons.X /> : <Icons.Menu />}
//               </button>
//             </div>
//           </div>
...
//         <AnimatePresence>
//           {isMenuOpen && (
//             <motion.div initial={{ opacity: 0, y: -20, scale: 0.95 }} animate={{ opacity: 1, y: 0, scale: 1 }} exit={{ opacity: 0, y: -20, scale: 0.95 }} transition={{ duration: 0.2 }} className={`md:hidden absolute left-4 right-4 mt-2 border rounded-lg shadow-2xl py-6 px-6 z-50 backdrop-blur-xl ${isDark ? "bg-[#0a0a0a] border-white/10" : "bg-white border-gray-200"}`}>
//               <ul className={`flex flex-col gap-2 text-lg font-medium ${isDark ? "text-gray-300" : "text-gray-800"}`}>
//                 {navLinks.map((link, i) => (
//                   <motion.li key={i} initial={{ x: -20, opacity: 0 }} animate={{ x: 0, opacity: 1 }} transition={{ delay: i * 0.05 }} className={`py-3 px-4 hover:bg-[#e1c693]/10 rounded-md cursor-pointer transition-colors ${isActiveLink(link) ? "bg-[#e1c693]/10 text-[#e1c693]" : ""}`}>
//                     <a href={link.href} onClick={(e) => handleNavigation(e, link.href, link.id)} className="flex items-center gap-4">{link.icon} {link.name}</a>
//                   </motion.li>
//                 ))}
//               </ul>
//             </motion.div>
...
//         {showThemePopup && (
//           <>
//             <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} onClick={() => setShowThemePopup(false)} className="fixed inset-0 bg-black/60 backdrop-blur-sm z-[100] cursor-pointer" />
//             <div className="fixed inset-0 flex items-center justify-center z-[101] pointer-events-none">
//               <motion.div variants={popupVariants} initial="hidden" animate="visible" exit="exit" onClick={(e) => e.stopPropagation()} className={`pointer-events-auto max-w-md mx-4 p-6 md:p-8 rounded-2xl border shadow-2xl ${isDark ? "bg-[#0a0a0a] border-[#e1c693]/30 text-gray-200" : "bg-white border-[#a78b54]/30 text-gray-900"}`}>
//                 <button onClick={() => setShowThemePopup(false)} className={`absolute top-4 right-4 p-2 rounded-full transition-colors ${isDark ? "hover:bg-white/10 text-gray-400 hover:text-[#e1c693]" : "hover:bg-gray-100 text-gray-500 hover:text-[#a78b54]"}`}><X className="w-5 h-5" /></button>
//                 <div className="flex justify-center mb-4">
//                   <div className={`w-16 h-16 rounded-full flex items-center justify-center ${isDark ? "bg-[#e1c693]/10 text-[#e1c693]" : "bg-[#a78b54]/10 text-[#a78b54]"}`}><Info className="w-8 h-8" /></div>
//                 </div>
//                 <h3 className={`text-xl font-bold text-center mb-3 ${isDark ? "text-white" : "text-gray-900"}`}>Theme Toggle Notice</h3>
//                 <p className={`text-sm md:text-base text-center leading-relaxed ${isDark ? "text-gray-400" : "text-gray-600"}`}>
//                   The dark and light mode functionality has been implemented <span className="font-semibold text-[#e1c693]">for testing purposes only</span> on the <span className="font-semibold text-[#e1c693]">hero section</span>. Full theme support will be added later.
//                 </p>
//                 <div className="mt-6 flex justify-center">
//                   <button onClick={() => setShowThemePopup(false)} className="px-6 py-2.5 bg-gradient-to-r from-[#e1c693] to-[#a78b54] text-[#0a0a0a] font-semibold rounded-full hover:shadow-lg hover:shadow-[#e1c693]/30 transition-all duration-300">Got it!</button>
//                 </div>
//               </motion.div>
//             </div>
//           </>
